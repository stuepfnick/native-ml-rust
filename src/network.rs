...
/// Classic `Layer` / `Neuron` graph: one `Layer` per weight matrix.
pub struct Network {
...
impl Network {
    ...
    pub fn new(layer_sizes: &[usize]) -> Self {
        if layer_sizes.len() < 2 {
            panic!("Network must have at least an input size and an output layer");
        }
    ...
    fn backpropagate(&mut self, activations: Vec<Vec<f32>>, targets: &[f32], learning_rate: f32) {
        ...
        for (i, layer) in self.layers.iter_mut().enumerate().rev() {
            ...
            let outputs = &activations[i + 1];
            current_errors = layer.update(inputs, outputs, &current_errors, learning_rate);
        }
    }

    pub fn print(&self) {
        println!("Network (legacy): {} layers", self.layers.len());
